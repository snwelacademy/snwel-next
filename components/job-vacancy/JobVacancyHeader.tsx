...
export default function JobVacancyHeader() {
  ...
  return (
    <header
      ...
      style={{
        backgroundImage: "url('/assets/images/job_vaccency.png')",
        backgroundSize: "cover",
        backgroundPosition: "center"
      }}
    >
      {/* ✅ Overlay for text readability (adjust or remove if needed) */}
      ...
      {/* ===== JOB TYPE LINKS ===== */}
      <motion.div
        className="relative bg-black/60 py-4"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ duration: 0.5, delay: 0.6 }}
      >
        <div className="container mx-auto px-4">
          <ul className="flex justify-center space-x-4 md:space-x-8 text-sm md:text-base">
            {[
              { label: "Remote Jobs", type: "remote" },
              { label: "Full-Time", type: "full-time" },
              { label: "Part-Time", type: "part-time" },
              { label: "Contract", type: "contract" }
            ].map(item => (
              <motion.li key={item.type} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Link
                  href={`/job-vacancies?type=${item.type}`}
