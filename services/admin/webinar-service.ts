...
export async function getAllPublicWebinars (options?: ListOptions) {
    try {
        options = {...DEFAULT_LIST_OPTIONS, ...options}
        const res = await api.get<any, AxiosResponse<ApiResponse<ListResponse<Webinar>>>>(`/webinar/open?${objectToQueryString(options)}`);
        const data = res.data.data;
        return data;
    } catch (error) {
        console.log("Error: getAllWebinar: ", error);
